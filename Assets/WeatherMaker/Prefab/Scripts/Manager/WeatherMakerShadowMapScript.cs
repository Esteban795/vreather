...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class WeatherMakerShadowMapScript : MonoBehaviour
    {
        ...
        private CameraEvent urpEventCascadeShadows = CameraEvent.BeforeDepthTexture;
        ...
        private void AddScreenSpaceShadowsCommandBuffer(Camera camera)
        {
                ...
                WeatherMakerScript.Instance.PerformanceProfile.VolumetricCloudShadowSampleCount > 0)
            {
                ...
                if (!string.IsNullOrWhiteSpace(ShaderTextureName))
                {
                    var texName = ShaderTextureName;
                    WeatherMakerCommandBufferManagerScript.Instance.AddCameraCommandBufferAction((ctx) =>
                    {
                        ctx.CommandBuffer.SetGlobalTexture(texName, BuiltinRenderTextureType.CurrentActive);
                    }, camera, urpEventCascadeShadows);
                }
        ...
        private void CameraPostRender(Camera camera)
        {
                ...
                && WeatherMakerScript.Instance.PerformanceProfile.VolumetricCloudReflectionShadows))
            {
                ...
                if (WeatherMakerCommandBufferManagerScript.Instance != null)
                {
                    ...
                    var cascadeActions = WeatherMakerCommandBufferManagerScript.Instance.GetCameraCommandBufferActions(camera, urpEventCascadeShadows);
                    if (cascadeActions is List<System.Action<WeatherMakerCommandBufferContext>> cascadeList)
                    {
                        cascadeList.Clear();
                    }
