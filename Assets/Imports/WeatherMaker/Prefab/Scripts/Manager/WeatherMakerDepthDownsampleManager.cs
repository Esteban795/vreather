...
namespace DigitalRuby.WeatherMaker
{
    ...
    {
        private static readonly int CameraDepthTextureHalfTexelSize = Shader.PropertyToID("_CameraDepthTextureHalf_TexelSize");
        private static readonly int CameraDepthTextureQuarterTexelSize = Shader.PropertyToID("_CameraDepthTextureQuarter_TexelSize");
        private static readonly int CameraDepthTextureEighthTexelSize = Shader.PropertyToID("_CameraDepthTextureEighth_TexelSize");
        private static readonly int CameraDepthTextureHalfST = Shader.PropertyToID("_CameraDepthTextureHalf_ST");
        private static readonly int CameraDepthTextureQuarterST = Shader.PropertyToID("_CameraDepthTextureQuarter_ST");
        private static readonly int CameraDepthTextureEighthST = Shader.PropertyToID("_CameraDepthTextureEighth_ST");
...
#else
            SetSharedDepthGlobals(state, desc2, desc4, desc8, null, false);
#endif
        }
...
#else
            BindGlobals(null, camera);
            if (state.CommandBuffer == null)
            {
                state.CommandBuffer = owner.GetOrCreateCommandBuffer();
            }
            BuildCommands(state.CommandBuffer, camera, depthDownsampleMaterial, state, true, false);
            owner.ReplaceCameraCommandBuffer(state.CommandBuffer, camera, CameraEvent.AfterDepthTexture);
#endif
        }
            ...
            RenderTextureDescriptor desc8, CommandBuffer commandBuffer, bool useCommandBufferGlobals)
        {
            ...
            if (useCommandBufferGlobals && commandBuffer != null)
            {
                ...
                commandBuffer.SetGlobalVector(CameraDepthTextureHalfST, st);
                commandBuffer.SetGlobalVector(CameraDepthTextureQuarterST, st);
                commandBuffer.SetGlobalVector(CameraDepthTextureEighthST, st);
                commandBuffer.SetGlobalVector(CameraDepthTextureHalfTexelSize, halfTexelSize);
                commandBuffer.SetGlobalVector(CameraDepthTextureQuarterTexelSize, quarterTexelSize);
                commandBuffer.SetGlobalVector(CameraDepthTextureEighthTexelSize, eighthTexelSize);
            }
            else
            {
                ...
                Shader.SetGlobalVector(CameraDepthTextureHalfST, st);
                Shader.SetGlobalVector(CameraDepthTextureQuarterST, st);
                Shader.SetGlobalVector(CameraDepthTextureEighthST, st);
                Shader.SetGlobalVector(CameraDepthTextureHalfTexelSize, halfTexelSize);
                Shader.SetGlobalVector(CameraDepthTextureQuarterTexelSize, quarterTexelSize);
                Shader.SetGlobalVector(CameraDepthTextureEighthTexelSize, eighthTexelSize);
            }
        }
    }
}
