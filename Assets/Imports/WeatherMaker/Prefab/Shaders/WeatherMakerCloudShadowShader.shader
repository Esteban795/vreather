...
Shader "WeatherMaker/WeatherMakerCloudShadowShader"
{
	Properties
	{
		...
		_WeatherMakerFogShadowFinalStrength("Fog Shadow Final Strength", Range(0.0, 1.0)) = 1.0
	}
	SubShader
	{
		...
		uniform float _WeatherMakerFogShadowFinalStrength;
		...
		Pass
		{
			...
			float4 shadowFrag(wm_full_screen_fragment i) : SV_Target
			{
				...
				if (depth01 < 1.0)
				{
					...
					float existingShadow = WM_SAMPLE_FULL_SCREEN_TEXTURE(_ScreenSpaceShadowmapTexture, i.uv.xy).r;
					float shadowWithCascades = ComputeCloudShadowStrength(worldPos, 0, existingShadow, _CloudShadowDetails, 1.0); //ComputeCloudShadowStrengthTexture(worldPos, 0, existingShadow, false);
					return min(shadowWithCascades, _WeatherMakerFogShadowFinalStrength);
				}
				else
				{
					return 1.0;
