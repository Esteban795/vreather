...
namespace DigitalRuby.WeatherMaker
{
    ...
    public static class WeatherMakerEditorCommands
    {
        ...
        private const string pendingURPFeatureSyncKey = "WeatherMaker_PendingURPFeatureSync";
        private const string suppressAutoEnableURPKey = "WeatherMaker_SuppressAutoEnableURP";
        private const string allowAutoEnableURPOnceKey = "WeatherMaker_AllowAutoEnableURPOnce";

        private static readonly string[] urpRendererAssetNames = new[]
        {
            "WeatherMakerURPDeferredRenderer",
            "WeatherMakerURPForwardRenderer"
        };

        private static Type GetWeatherMakerURPRenderFeatureType()
        {
            return AppDomain.CurrentDomain
                .GetAssemblies()
                .Select(a => a.GetType("DigitalRuby.WeatherMaker.WeatherMakerURPRenderFeatureScript", false))
                .FirstOrDefault(t => t != null);
        }

        private static bool HasWeatherMakerRenderFeature(SerializedProperty rendererFeatures, Type featureType)
        {
            for (int i = 0; i < rendererFeatures.arraySize; i++)
            {
                UnityEngine.Object featureObj = rendererFeatures.GetArrayElementAtIndex(i).objectReferenceValue;
                if (featureObj != null && featureObj.GetType() == featureType)
                {
                    return true;
                }
            }
        ...
        private static void RebuildRendererFeatureMap(SerializedObject so, SerializedProperty rendererFeatures)
        {
            SerializedProperty rendererFeatureMap = so.FindProperty("m_RendererFeatureMap");
            ...
            for (int i = 0; i < rendererFeatures.arraySize; i++)
            {
                ...
                if (featureObj != null)
                {
                    AssetDatabase.TryGetGUIDAndLocalFileIdentifier(featureObj, out string _, out localId);
                }
                rendererFeatureMap.GetArrayElementAtIndex(i).longValue = localId;
            }
        }

        private static void AddWeatherMakerRenderFeatureToRenderer(ScriptableObject rendererData, Type featureType)
        {
            ...
            if (!HasWeatherMakerRenderFeature(rendererFeatures, featureType))
            {
                ScriptableObject featureObj = ScriptableObject.CreateInstance(featureType);
                featureObj.name = "WeatherMakerURPRenderFeatureScript";
                AssetDatabase.AddObjectToAsset(featureObj, rendererData);
                AssetDatabase.ImportAsset(AssetDatabase.GetAssetPath(featureObj));

                int index = rendererFeatures.arraySize;
                rendererFeatures.InsertArrayElementAtIndex(index);
                rendererFeatures.GetArrayElementAtIndex(index).objectReferenceValue = featureObj;
                RebuildRendererFeatureMap(so, rendererFeatures);
                so.ApplyModifiedPropertiesWithoutUndo();
                EditorUtility.SetDirty(rendererData);
            }
        }

        private static void RemoveWeatherMakerRenderFeatureFromRenderer(ScriptableObject rendererData, Type featureType)
        {
            SerializedObject so = new SerializedObject(rendererData);
            SerializedProperty rendererFeatures = so.FindProperty("m_RendererFeatures");
            if (rendererFeatures == null)
            ...
            for (int i = rendererFeatures.arraySize - 1; i >= 0; i--)
            {
                ...
                if (featureObj != null && featureObj.GetType() == featureType)
                {
                    ...
                    if (i < rendererFeatures.arraySize && rendererFeatures.GetArrayElementAtIndex(i).objectReferenceValue == null)
                    {
                        rendererFeatures.DeleteArrayElementAtIndex(i);
                    }
                    UnityEngine.Object.DestroyImmediate(featureObj, true);
                }
            }
        ...
        private static bool SyncWeatherMakerURPRenderFeatures(bool enable)
        {
            Type featureType = GetWeatherMakerURPRenderFeatureType();
            if (featureType == null)
            {
                return false;
            }

            foreach (string rendererName in urpRendererAssetNames)
            {
                string[] rendererGuids = AssetDatabase.FindAssets(rendererName);
                foreach (string guid in rendererGuids)
                {
                    string path = AssetDatabase.GUIDToAssetPath(guid);
                    ScriptableObject rendererData = AssetDatabase.LoadAssetAtPath<ScriptableObject>(path);
                    if (rendererData == null || !rendererData.name.Equals(rendererName, StringComparison.Ordinal))
                    {
                        continue;
                    }
                    ...
                    if (enable)
                    {
                        AddWeatherMakerRenderFeatureToRenderer(rendererData, featureType);
                    }
                    else
                    {
                        RemoveWeatherMakerRenderFeatureFromRenderer(rendererData, featureType);
                    }
                }
            }
        ...
        {
            RenderPipelineAsset pipeline = UnityEngine.Rendering.GraphicsSettings.defaultRenderPipeline;
            if (pipeline == null)
            {
                pipeline = UnityEngine.QualitySettings.renderPipeline;
            }
        ...
        private static void ApplyPendingURPFeatureSync()
        {
            if (!EditorPrefs.GetBool(pendingURPFeatureSyncKey, false))
            {
                return;
            }

            // If user switched to standard (or non-URP) pipeline, never apply pending URP feature sync.
            if (!IsUniversalRenderPipelineActive())
            {
                EditorPrefs.DeleteKey(pendingURPFeatureSyncKey);
                return;
            }

            if (SyncWeatherMakerURPRenderFeatures(true))
            {
                EditorPrefs.DeleteKey(pendingURPFeatureSyncKey);
                Debug.Log("Weather Maker: Added URP render feature to WeatherMakerURPForwardRenderer and WeatherMakerURPDeferredRenderer.");
            }
        }
        ...
        [MenuItem("Window/Weather Maker/Enable Standard Render Pipeline", false, priority = 43)]
        public static void EnableStandardPipeline()
        {
            EditorPrefs.SetBool(suppressAutoEnableURPKey, true);
            EditorPrefs.SetBool(allowAutoEnableURPOnceKey, false);
            UnityEngine.Rendering.GraphicsSettings.defaultRenderPipeline = null;
            UnityEngine.QualitySettings.renderPipeline = null;
            SyncWeatherMakerURPRenderFeatures(false);
            EditorPrefs.DeleteKey(pendingURPFeatureSyncKey);
            ...
            AssetDatabase.Refresh();
            UnityEditor.EditorUtility.DisplayDialog("Success", "Standard pipeline enabled. Please wait for script recompile.", "OK");
        }

        [MenuItem("Window/Weather Maker/Enable Universal Render Pipeline", false, priority = 44)]
        public static void EnableURP()
        {
            EditorPrefs.SetBool(suppressAutoEnableURPKey, false);
            EditorPrefs.SetBool(allowAutoEnableURPOnceKey, true);
            EnableURPInternal("UnityEngine.Rendering.Universal.UniversalRenderPipeline",
                "WeatherMakerURPProfile",
                "Please import URP package first.",
                "Unable to find WeatherMakerURPProfile, please re-download from asset store.",
                "URP enabled. Please ensure render features are assigned properly. You need screen space shadows as the first render feature and Weather Maker as the second. " +
                "If you are seeing strange errors, please re-create the URP forward renderer, add screen space shadows and then Weather Maker render features and " +
                "then re-assign to the URP render asset.");
        }
        ...
        private static void EnableURPInternal(string type, string profile, string importErrorText, string profileErrorText, string renderFeatureText)
        {
            ...
            if (!SyncWeatherMakerURPRenderFeatures(true))
            {
                // If UNITY_URP scripts are not compiled yet, defer feature sync to next domain reload.
                EditorPrefs.SetBool(pendingURPFeatureSyncKey, true);
            }
            UnityEditor.EditorUtility.DisplayDialog("Notice", "URP pipeline enabled. Please wait for script recompile.", "OK");
        }
        ...
        public static void Extract3DTexture()
        {
            ...
            SerializationHelper.Extract3DTexture(tex3D, docsPath);
